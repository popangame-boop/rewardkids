...
interface ParentDashboardClientProps {
  ...
  initialMissionsCount: number;
  initialRewardsCount: number;
  initialRecentLedgers: RecentLedger[];
  parentId: string;
  parentName: string;
}
...
export function ParentDashboardClient({
  ...
  // Sync all dashboard data using SWR
  const { data: dashboardData, mutate: mutateDashboard } = useSWR(
    ["parentDashboardData", parentId],
    async () => {
      // 1. Fetch children
      ...
      return {
        ...
        missionsCount: missions ?? 0,
        rewardsCount: rewards ?? 0,
        recentLedgers: (ledgers as RecentLedger[]) ?? [],
      };
    },
    {
      fallbackData: {
        ...
        missionsCount: initialMissionsCount,
  ...
  useEffect(() => {
    const channel = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "profiles" }, () => { mutateDashboard(); })
      .on("postgres_changes", { event: "*", schema: "public", table: "ledgers" }, () => { mutateDashboard(); })
      .on("postgres_changes", { event: "*", schema: "public", table: "missions" }, () => { mutateDashboard(); })
      .on("postgres_changes", { event: "*", schema: "public", table: "rewards" }, () => { mutateDashboard(); })
      .subscribe();
                ...
                >
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-fun-text/60 text-xs truncate font-semibold mt-0.5">
                      {ledger.profiles?.name} • {ledger.points} poin
                    </p>
                  </div>
